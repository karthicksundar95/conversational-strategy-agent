...
def load_prompt(path: str) -> str:
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
        # Strip the first line if it's "prompt = f""""
        if content.startswith('prompt = f"""'):
            content = content[len('prompt = f"""'):].lstrip()
        # Also handle case where it might end with """ (though it shouldn't in template files)
        return content

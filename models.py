...
class SearchDocumentsOutput(BaseModel):
    result: List[str]  # List of document chunks
...
class AnswerFromHistoryInput(BaseModel):
    query: str
    historical_context: str = Field(default="", description="Formatted historical conversation context")

class AnswerFromHistoryOutput(BaseModel):
    result: str  # FINAL_ANSWER format

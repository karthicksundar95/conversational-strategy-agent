        ...
        for config in self.server_configs:
            try:
                ...
                async with stdio_client(params) as (read, write):
                    #print("Connection established, creating MCP client session...")
                    try:
                        async with ClientSession(read, write) as session:
                            #print("[agent] MCP Session created, initializing...")
                            await session.initialize()
                            print(f"[agent] MCP session for {config['script']} initialized")

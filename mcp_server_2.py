...
import time
from models import AddInput, AddOutput, SqrtInput, SqrtOutput, StringsToIntsInput, StringsToIntsOutput, ExpSumInput, ExpSumOutput, PythonCodeInput, PythonCodeOutput, UrlInput, FilePathInput, MarkdownInput, MarkdownOutput, ChunkListOutput, SearchDocumentsInput, SearchDocumentsOutput
...
@mcp.tool()
def search_stored_documents(input: SearchDocumentsInput) -> SearchDocumentsOutput:
    ...
    try:
        ...
        return SearchDocumentsOutput(result=results)
    except Exception as e:
        return SearchDocumentsOutput(result=[f"ERROR: Failed to search: {str(e)}"])
...
def caption_image(img_url_or_path: str) -> str:
    ...
    try:
        # Check if it's a URL first (before trying local path)
